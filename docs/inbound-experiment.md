...
## Discovery experiment 3 — September 21, 2026

Hypothesis: people seeing the existing `/california-qsbs/` guide for California QSBS searches are more likely to visit when its search title states the answer (no state exclusion) and its description promises a concrete federal-versus-state example. The page now has a static, sourced comparison table for a fully federally excluded $2 million gain and a matching link to the existing calculator. No new competing URL or financial-input collection is introduced.

Distribution mechanism: organic search on an already surfaced canonical California guide. This is a bounded search-presentation and answer experiment, not a claim that the title will be displayed verbatim or that publishing guarantees distribution. The table is accessible in initial HTML. No indexing request is required. The existing eligibility checklist and its observation window remain unchanged.
